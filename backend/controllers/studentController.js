...
const formatStudent = (record) => {
    if (!record || !record.has('s')) return null;

    const node = record.get('s').properties;
    const courses = record.has('courses') ? record.get('courses') : [];
    const programName = record.has('programName') ? record.get('programName') : null;
    ...
    try {
        const result = await session.run(
            `MATCH (s:Student)
             ...
             WITH s, collect(c) AS courses, p.name AS programName
             RETURN s, courses, programName
             ORDER BY s.createdAt DESC`
        );
        const students = result.records.map(formatStudent).filter(s => s !== null);
        res.json(students);
    } catch (error) {
        console.error(error); 
...
const createStudent = async (req, res) => {
    const session = getDriver().session();
    try {
        const { studentId, name, email, courses, program } = req.body;
        const id = uuidv4();
